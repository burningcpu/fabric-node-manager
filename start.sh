...
APP_MAIN=com.webank.fabric.node.manager.Application
...
start(){
    ...
    if [ $processStatus == 1 ]; then
        ...
        nohup $JAVA_HOME/bin/java -Djdk.tls.namedGroups="secp256k1" $JAVA_OPTS -Djava.library.path=$CONF_DIR -cp $CLASSPATH $APP_MAIN >> $LOG_DIR/manager.out 2>&1 &
